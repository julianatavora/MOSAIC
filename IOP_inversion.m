...
ap_NIRoffset = 0.0072; %offset as in Rottgers et al 2014
offset_band = 850;
for i = 1:length(Snap)
...
if any(~isnan(rrs_pix))
    ...
    for q = 1:length(temp_pix)
        ...
        % Solve all K least-squares systems
        p_matrix = zeros(4, K);
        for k = 1:K
        ...
        if ~isempty(B)
            ...
            if num_valid > 0
                ...
                bbp_exp_model = bb_p_ori(nn_vec(valid_B), :);      
...
for i = 1:size(a_f,1)
    a_phi(i,:)=interp1(wl,a_f(i,:),wavelength,'linear','extrap')';
end
